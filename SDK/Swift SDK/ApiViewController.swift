...
class ApiViewController: CommonViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let modelIndex: ModelIndex?       = AppDelegate.getSelectedModelIndex()
        ...
        case 8:
            commands = ApiFunctions.createTopMarginData(modelIndex, emulation)
        case 9:
        ...
        case 27:
            self.showLabelTypeSelectionAlert { (optLabelMarkType) in
                guard let labelType = optLabelMarkType else {
                    return
                }
                
                let emulation = AppDelegate.getEmulation()
                
                let commands = ApiFunctions.createLabelData(emulation, type: labelType)
    ...
    func showLabelTypeSelectionAlert(completion:@escaping (SCBLabelType?) -> Void) {
        let alert = UIAlertController(title: "Select label type.",
                                      message: nil,
                                      preferredStyle: .alert)
        
        func addSelection(title: String, style: UIAlertAction.Style, labelType: SCBLabelType?) {
            let alertAction = UIAlertAction(title: title, style: .default, handler: { _ in
                DispatchQueue.main.async {
                    completion(labelType)
                }
            })
        ...
        addSelection(title: "Invalid", style: .default, labelType: .invalid)
        addSelection(title: "Valid", style: .default, labelType: .valid)
        addSelection(title: "Valid with Detection", style: .default, labelType: .validWithDetection)
        addSelection(title: "Cancel", style: .cancel, labelType: nil)
