...
class BluetoothSettingViewController: CommonViewController, UITableViewDelegate, UITableViewDataSource, TextViewTableViewCellDelegate, SwitchTableViewCellDelegate {
    ...
    var isSMSorTSeries: Bool!
    var fwInfoSMSorTSeries: FwInfoSMSorTSeries!
    ...
    override func viewDidLoad() {
        ...
        self.isSMSorTSeries = false
        self.fwInfoSMSorTSeries = .v4orEalier
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if didAppear == false {
            ...
            if (modelName.hasPrefix("SM-S21") ||
                ...
                if (NSString(string: firmwareVersion).doubleValue < 5.0) {
                    self.fwInfoSMSorTSeries = .v4orEalier
                }
                else {
                    self.fwInfoSMSorTSeries = .v5orLater
                }
            }
...
//      case 3 :
            ...
            if (self.bluetoothManager.securityTypeCapability == SMBluetoothSettingCapabilitySupport) {
                if (self.bluetoothManager.security == SMBluetoothSecuritySSP) {
                    if (self.bluetoothManager.deviceType == SMDeviceTypeDesktopPrinter) {
                        ...
                        cell.detailTextLabel!.text = "SSP Just Works"
                    }
                }
                else if (self.bluetoothManager.security == SMBluetoothSecurityPINcode) {
                    cell.detailTextLabel!.text = "PIN Code"
                }
                else if (self.bluetoothManager.security == SMBluetoothSecuritySSPNumericComparison) {
                    cell.detailTextLabel!.text = "SSP Numeric Comparison"
                }
                else {  // Disable
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (indexPath.row == 3) {
            if (self.bluetoothManager.securityTypeCapability == SMBluetoothSettingCapabilitySupport) {
                ...
                if (!self.isSMSorTSeries && !self.isSMLSeries) {
                    self.showSecuritySelectionAlert(deviceType: self.bluetoothManager.deviceType)
                }
                else if (self.isSMSorTSeries && self.fwInfoSMSorTSeries == .v5orLater) {
                    self.showSecuritySelectionAlert(deviceType: self.bluetoothManager.deviceType)
                }
                else {
                }
    ...
    func showSecuritySelectionAlert(deviceType: SMDeviceType) {
        ...
        case SMDeviceTypePortablePrinter:
            addSecuritySelection(title: "SSP Just Works", securitySetting: SMBluetoothSecuritySSP)
            addSecuritySelection(title: "SSP Numeric Comparison", securitySetting: SMBluetoothSecuritySSPNumericComparison)
            addSecuritySelection(title: "PIN Code", securitySetting: SMBluetoothSecurityPINcode)
            //addSecuritySelection(title: "Disable", securitySetting: SMBluetoothSecurityDisable) // SMBluetoothSecurityDisable is deprecated.
            alert.addAction(cancelAction)
        default:
            preconditionFailure()
            break
